 ...
 * @LastEditTime: 2024-06-23 12:51:41
...
function zibpay_mail_payment_order_to_referrer($values)
{
    ...
    if ($wechat_template_id) {
        ...
        $send_data = array(
            ...
            'keyword2' => array(
                'value' => $rebate_price . '元' . '（您当前累计佣金' . $all_rebate_price . '元，待提现' . $rebate_effective_data . '元）',
            ),
            'keyword3' => array(
                'value' => $pay_order['pay_time'],
            ),
...
function zibpay_msg_apply_withdraw_to_admin($msg_args)
{
    ...
    if ($wechat_template_id) {
        ...
        $send_data = array(
            ...
            'keyword2' => array(
                'value' => (string) floatval($msg_args['meta']['withdraw_price']) . '元',
            ),
            'keyword3' => array(
...
function zibpay_wechat_template_msg_withdraw_process($msg_db, $is_allow, $msg = '')
{
    ...
    $send_data = array(
        ...
        'keyword3' => array(
            'value' => '您的提现申请' . ($is_allow ? '已处理完成' : '被拒绝') . ($msg ? '，处理留言：' . $msg : ''),
            "color" => ($is_allow ? '#1a7dfd' : '#fd4343'),
        ),
        'keyword4' => array(
            'value' => (string) floatval(round((float) $msg_db['meta']['withdraw_price'], 2)),
        ),
        'keyword5' => array(
            'value' => (string) floatval(round((float) (($is_allow ? ($msg_db['meta']['withdraw_price'] - (isset($msg_db['meta']['service_price']) ? $msg_db['meta']['service_price'] : 0)) : '0')))),
        ),
        'remark'   => array(
            'value' => ($msg ? '处理留言：' . $msg . "\n" : '') . '如有疑问，请与客服联系',
        ),
